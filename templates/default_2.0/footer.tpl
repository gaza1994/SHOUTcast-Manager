</section>
</aside>
        <footer class="main-footer">
            <div class="pull-right hidden-xs">
                <b>Version</b> 2.0.0
            </div>
            <strong>SHOUTcast Manager Copyright &copy; 2016 <a href="http://scottishbordersdesign.co.uk" target=_blank>Scottish Borders Design</a>.</strong> All rights reserved.
        </footer>

        <script src="//ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>
        <script src="{$template_dir}/js/jquery-ui-1.10.3.min.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/bootstrap.min.js" type="text/javascript"></script>
        <script src="//cdnjs.cloudflare.com/ajax/libs/raphael/2.1.0/raphael-min.js"></script>
        <script src="{$template_dir}/js/plugins/moment/moment.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/sparkline/jquery.sparkline.min.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/jvectormap/jquery-jvectormap-1.2.2.min.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/jvectormap/jquery-jvectormap-world-mill-en.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/fullcalendar/fullcalendar.min.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/jqueryKnob/jquery.knob.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/daterangepicker/daterangepicker.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/bootstrap-wysihtml5/bootstrap3-wysihtml5.all.min.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/AdminLTE/app.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/app.min.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/datatables/jquery.dataTables.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/plugins/datatables/dataTables.bootstrap.js" type="text/javascript"></script>
        <script src="{$template_dir}/js/AdminLTE/dashboard.js" type="text/javascript"></script> 
    </body>
</html>