...
$SHOUTcastPort = $myserver['PortBase'];
...
if ($showHistory == 1) {
    ...
    curl_close($ch);
    if ($SHOUTcastData) {
        print 'document.getElementById(\'SHOUTcastHistory\').innerHTML="<table class=\"'.$tableClass.'\">';
        ...
        foreach ($matches[1] as $val) {
            if ($val != "") {
                ...
                if ($show) {
                    ...
                    print utf8_encode(str_replace('"', '\"', "  <tr><td><a title='Search Google for this track.' target=_blank href='https://www.google.co.uk/search?q={$val}&oq=search'>{$val}</a></td></tr>"));
                }
                $i++;
            }
        }
